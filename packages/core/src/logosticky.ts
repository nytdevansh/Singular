...
export class StickyLogoManager {
  ...
  private activeStickyLogos: Map<string, any> = new Map();

  initializeStickyLogo(logoSelector: string, options: any = {}) {
    const logoEl = document.querySelector(logoSelector) as HTMLElement;
    if (!logoEl) {
  ...
  destroyStickyLogo(logoSelector: string) {
